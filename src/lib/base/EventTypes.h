...
enum class EventTypes : uint32_t
{
  ...
  */
  ...
  // --- v1.9 drag-and-drop file transfer ---

  /** Sent by the primary screen's low-level hook when the left mouse button is
      pressed AND the cursor has entered the jump zone with an active OLE drag
      payload. Event data is a pointer to DragStartInfo (list of source paths).
  */
  PrimaryScreenDragStart,

  /// Sender-side: a FileChunk has been produced and is ready to be written to
  /// the network stream. Event data is a pointer to FileChunk.
  FileChunkSending,

  /// Receiver-side: a FileChunk has been assembled from the incoming stream.
  /// Event data is a pointer to FileChunk.
  FileChunkReceived,

  /// A drag-and-drop transfer has begun. Event data is a pointer to
  /// TransferInfo { id, count, totalBytes }.
  DragTransferStarted,

  /// Progress heartbeat, emitted at most every 100 ms or once per chunk.
  /// Event data is a pointer to TransferProgress.
  DragTransferProgress,

  /// A drag-and-drop transfer completed successfully or with a per-file error.
  /// Event data is a pointer to TransferResult.
  DragTransferFinished,

  /// A drag-and-drop transfer was aborted (user cancel, protocol error, ...).
  /// Event data is a pointer to TransferAbortInfo.
  DragTransferAborted,

  /// Receiver got a kMsgQDropTargetInfo query and needs to reply with the
  /// hover target directory + conflict policy. Data: TargetQueryInfo.
  DragTargetQuery,
