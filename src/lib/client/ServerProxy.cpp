...
ServerProxy::ConnectionResult ServerProxy::parseMessage(const uint8_t *code)
{
  ...
  // --- v1.9 drag-and-drop file transfer (Stage 1: consume & log only) ------
  else if (memcmp(code, kMsgDDragInfo, 4) == 0) {
    handleDragInfo();
  }

  else if (memcmp(code, kMsgDFileTransferMeta, 4) == 0) {
  ...
  else if (memcmp(code, kMsgQDropTargetInfoQuery, 4) == 0) {
    handleDropTargetQuery();
  }

  else if (memcmp(code, kMsgQDragCancel, 4) == 0) {
    handleDragCancel();
  }

  else if (memcmp(code, kMsgDFileTransferProgress, 4) == 0) {
...
// ---------------------------------------------------------------------------
// v1.9 drag-and-drop file transfer -- Stage 1 skeleton
//
// The client-side handlers parse each frame and log its contents. In Stage 3
// they will forward the parsed fields to the local FileTransferManager and
// (for target drops) MSWindowsDropTarget.
// ---------------------------------------------------------------------------
...
void ServerProxy::handleDragInfo()
{
  ...
  if (!ProtocolUtil::readf(
          m_stream, kMsgDDragInfo + 4, &fileCount, &transferId, &totalHi, &totalLo, &manifest
      )) {
    LOG_WARN("failed to parse DDRG from server");
    return;
  }
  LOG_DEBUG(
      "recv drag info from server: count=%u xferId=%u totalSize=%llu manifestBytes=%zu",
  ...
  // Stage 3: m_client->onDragInfoReceived(transferId, fileCount, total, manifest);
}

void ServerProxy::handleFileTransferMeta()
{
  uint32_t transferId = 0;
  ...
  std::string relPath;
  if (!ProtocolUtil::readf(
          m_stream, kMsgDFileTransferMeta + 4, &transferId, &fileIndex, &sizeHi, &sizeLo, &flags, &relPath
      )) {
    LOG_WARN("failed to parse DFTM from server");
    return;
  }
  uint64_t size = (static_cast<uint64_t>(sizeHi) << 32) | sizeLo;
  LOG_DEBUG(
      "recv file meta from server: xferId=%u idx=%u size=%llu flags=0x%02x path=%s",
...
void ServerProxy::handleFileChunk()
{
  uint8_t mark = 0;
  std::string data;
  if (!ProtocolUtil::readf(m_stream, kMsgDFileTransfer + 4, &mark, &data)) {
    LOG_WARN("failed to parse DFTR from server");
    return;
  }
  LOG_DEBUG("recv file chunk from server: mark=%u bytes=%zu", static_cast<unsigned>(mark), data.size());
  // Stage 3: FileTransferManager::onChunk(mark, data)
}

void ServerProxy::handleDropTargetQuery()
{
  // The frame comes in as the query form: "QDTI%4i%2i%2i".
  // On the client we're the receiver, so we should reply with the local
  // drop target directory + conflict strategy. Stage 3 will wire the reply.
  uint32_t transferId = 0;
  uint16_t hoverX = 0;
  uint16_t hoverY = 0;
  if (!ProtocolUtil::readf(m_stream, kMsgQDropTargetInfoQuery + 4, &transferId, &hoverX, &hoverY)) {
    LOG_WARN("failed to parse QDTI query from server");
    return;
  }
  LOG_DEBUG(
      "recv drop-target query from server: xferId=%u hover=(%d,%d)", static_cast<unsigned>(transferId),
      static_cast<int16_t>(hoverX), static_cast<int16_t>(hoverY)
  );
  // Stage 3: m_client->onDropTargetQuery(transferId, hoverX, hoverY);
}

void ServerProxy::handleDragCancel()
{
  uint32_t transferId = 0;
  uint8_t reason = 0;
  if (!ProtocolUtil::readf(m_stream, kMsgQDragCancel + 4, &transferId, &reason)) {
    LOG_WARN("failed to parse QDGX from server");
    return;
  }
  LOG_DEBUG(
      "recv drag cancel from server: xferId=%u reason=%u", static_cast<unsigned>(transferId),
      static_cast<unsigned>(reason)
  );
  // Stage 3: FileTransferManager::onAbort(transferId, reason)
}

void ServerProxy::handleFileTransferProgress()
{
  uint32_t transferId = 0;
  uint32_t fileIndex = 0;
  uint32_t doneHi = 0;
  ...
  uint32_t percent = 0;
  if (!ProtocolUtil::readf(
          m_stream, kMsgDFileTransferProgress + 4, &transferId, &fileIndex, &doneHi, &doneLo, &percent
      )) {
    LOG_WARN("failed to parse DFPR from server");
    return;
  }
  uint64_t done = (static_cast<uint64_t>(doneHi) << 32) | doneLo;
  LOG_DEBUG(
      "recv progress from server: xferId=%u idx=%u done=%llu percent=%u.%02u",
      static_cast<unsigned>(transferId), static_cast<unsigned>(fileIndex),
