...
struct ChunkType
{
  ...
  inline static const auto DataAbort = 4; ///< Transfer aborted (v1.9+, file transfer only)
};
 ...
 * @brief File transfer and drag-and-drop messages (v1.5+, extended in v1.9)
 * @{
 */
 ...
 *
 * **Transfer Marks (v1.9+)**:
 * - `1` (kDataStart): Data payload:
 *     - v1.5:  8-byte total size (single-file legacy)
 *     - v1.9+: `[transferId:4][sizeHi:4][sizeLo:4]` for the current file within a transfer
 * - `2` (kDataChunk): Data payload: `[transferId:4][bytes]`
 * - `3` (kDataEnd):   Data payload: `[transferId:4]` (may be empty on v1.5)
 * - `4` (kDataAbort): Data payload: `[transferId:4][reason:1]` (v1.9+ only)
 *
 * @see kMsgDDragInfo, kMsgDFileTransferMeta
 * @since Protocol version 1.5, extended in v1.9
 */
extern const char *const kMsgDFileTransfer;

/**
 * @brief Drag and drop information (manifest)
 *
 * **Message Code**: `"DDRG"`
 * **Direction**: Primary ↔ Secondary
 *
 * **Legacy v1.5 format**: `"DDRG%2i%s"` (fileCount, null-separated paths).
 *   ClientProxy1_5 kept this frame as a no-op; parse-only for compatibility.
 *
 * **v1.9 format**: `"DDRG%2i%4i%4i%4i%s"`
 *   Parameters:
 *   - `$1`: fileCount (uint16)
 *   - `$2`: transferId (uint32) — session-scoped monotonic
 *   - `$3`: totalSizeHi (uint32) — high 32 bits of total byte count
 *   - `$4`: totalSizeLo (uint32) — low 32 bits
 *   - `$5`: manifest blob — see DragInformation::marshal
 *
 * Sent when a drag-and-drop operation crosses screens. The receiver replies
 * with kMsgQDropTargetInfo once the user hovers a drop target; actual bytes
 * flow via kMsgDFileTransferMeta + kMsgDFileTransfer chunks.
 *
 * @see kMsgDFileTransferMeta, kMsgDFileTransfer, kMsgQDropTargetInfo
 * @since Protocol version 1.5, redefined in v1.9
 */
extern const char *const kMsgDDragInfo;

/**
 * @brief File transfer metadata (per-file preamble)
 *
 * **Message Code**: `"DFTM"`
 * **Direction**: Sender → Receiver (bidirectional)
 * **Format**: `"DFTM%4i%4i%4i%4i%1i%s"`
 * **Parameters**:
 * - `$1`: transferId (uint32)
 * - `$2`: fileIndex (uint32) — 0-based index within manifest
 * - `$3`: sizeHi (uint32)
 * - `$4`: sizeLo (uint32)
 * - `$5`: flags (uint8) — bit0=isDir, bit1=lastFile
 * - `$6`: relPath (string) — UTF-8, forward-slash separated
 *
 * Sent immediately before the DFTR chunk stream for each file, so that the
 * receiver knows the target path and total size before any bytes arrive.
 *
 * @see kMsgDFileTransfer, kMsgDDragInfo
 * @since Protocol version 1.9
 */
extern const char *const kMsgDFileTransferMeta;

/**
 * @brief Drop target information (reply)
 *
 * **Message Code**: `"QDTI"`
 ...
 * **Parameters**:
 *   - `$1`: transferId (uint32)
 *   - `$2`: conflictStrategy (uint8) — 0=ask, 1=overwrite, 2=keepBoth, 3=skip
 *   - `$3`: targetPath (string, UTF-8) — absolute directory on the receiver
 *
 * Sent in response to a kMsgQDropTargetInfoQuery ("QDTQ") frame. The sender
 * uses this reply to gate the actual byte stream; if no reply arrives within
 * a timeout the sender falls back to the receiver's default download
 * directory.
 *
 * @since Protocol version 1.9
 */
extern const char *const kMsgQDropTargetInfo;

/**
 * @brief Drop target information (query form)
 *
 * **Message Code**: `"QDTQ"`
 * **Direction**: Sender → Receiver
 * **Format**: `"QDTQ%4i%2i%2i"`
 * **Parameters**:
 * - `$1`: transferId (uint32)
 * - `$2`: hoverX (int16)
 * - `$3`: hoverY (int16)
 *
 * Sent by the drag source to ask the receiver where the drop should land.
 * The receiver replies with kMsgQDropTargetInfo (code "QDTI").
 * Two distinct 4-byte message codes are used so parseMessage can dispatch
 * with a single memcmp per frame.
 *
 * @since Protocol version 1.9
 */
extern const char *const kMsgQDropTargetInfoQuery;

/**
 * @brief Drag / file-transfer cancellation
 *
 * **Message Code**: `"QDGX"`
 ...
 * **Parameters**:
 * - `$1`: transferId (uint32)
 * - `$2`: reason (uint8) — 0=user, 1=escape, 2=conflict_skip_all, 3=io_error
 *
 * Either side can send this to abort a pending or in-progress transfer.
 * On receipt the peer must stop sending/writing, discard partial data, and
 * emit DragTransferAborted locally.
 *
 * @since Protocol version 1.9
 */
extern const char *const kMsgQDragCancel;

/**
 * @brief File transfer progress (optional, cross-host)
 *
 * **Message Code**: `"DFPR"`
 * **Direction**: Receiver → Sender
 * **Format**: `"DFPR%4i%4i%4i%4i%4i"`
 * **Parameters**:
 * - `$1`: transferId (uint32)
 * - `$2`: fileIndex (uint32)
 * - `$3`: doneHi (uint32) — bytes committed to disk, high 32 bits
 * - `$4`: doneLo (uint32) — low 32 bits
 * - `$5`: percent (uint32) — 0-10000 (100.00% granularity)
 *
 * Sent from the receiver back to the sender so the sender-side GUI can
 * display "acknowledged" progress. Primarily consumed locally via IPC; only
 * enabled across the wire when option `kOptionCrossHostProgress` is set.
 *
 * @since Protocol version 1.9
 */
extern const char *const kMsgDFileTransferProgress;
