...
public:
  ...
  */
  ...
  //! @name v1.9 file transfer (default no-op for older protocol versions)
  //@{
  //! Send per-file metadata preamble (v1.9+)
  virtual void
  sendFileTransferMeta(uint32_t transferId, uint32_t fileIndex, uint64_t size, uint8_t flags, const std::string &relPath)
  {
    (void)transferId;
    (void)fileIndex;
    (void)size;
  ...
  //! Ask the client where the drop should land (v1.9+)
  virtual void sendDropTargetQuery(uint32_t transferId, int16_t hoverX, int16_t hoverY)
  {
    (void)transferId;
    (void)hoverX;
    (void)hoverY;
  }
  //! Abort an in-progress transfer (v1.9+)
  virtual void sendDragCancel(uint32_t transferId, uint8_t reason)
  {
    (void)transferId;
    (void)reason;
  }
  //! Ack incremental byte progress (v1.9+, optional cross-host)
  virtual void
  sendFileTransferProgress(uint32_t transferId, uint32_t fileIndex, uint64_t done, uint32_t percent)
  {
    (void)transferId;
    (void)fileIndex;
    (void)done;
